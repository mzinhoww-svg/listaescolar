...
A regra do CLAUDE.md vale para **componente React** (250 linhas). Varredura de 2026-09-25, reconferida com `wc -l` sobre `b905cce` (contagens iguais; (`find app components features lib supabase/functions scripts -name '*.ts' -o -name '*.tsx' | xargs wc -l`, sem testes)): **nenhum `.tsx` passa de 250 linhas**, então a regra não é violada. Os arquivos abaixo são módulos (repositórios, motores, núcleo de IA) e entram como recomendação com o mesmo limiar de 250.
...
| D-072 | PR #20 (CI); ledger-comercio S27 (assets/fonts) | `pnpm build` do CI depende de `next/font/google` (busca Plus Jakarta Sans no Google Fonts): falhou uma vez por rede no PR #20 (rerun passou). Hospedar a fonte localmente (a S27 já commitou `assets/fonts` para a OG image) | média | S19 / S18 | aberta |
