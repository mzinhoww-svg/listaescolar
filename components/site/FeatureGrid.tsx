...
export function FeatureGrid({ items, dark = false }: Props) {
  return (
    <ul className="grid gap-4 md:grid-cols-3">
      {items.map((i) => (
        <li key={i.title} className={`rounded-card p-6 ${dark ? "bg-white/10" : "bg-white"}`}>
          <h3 className="text-lg font-extrabold">{i.title}</h3>
          <p className={`mt-2 text-[15px] leading-relaxed ${dark ? "text-papel/85" : "text-texto-2"}`}>{i.text}</p>
        </li>
      ))}
    </ul>
  );
