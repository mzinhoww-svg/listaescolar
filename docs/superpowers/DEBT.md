...
| D-060 | ledger-pipeline S07 T2; PR #14; verificação de 2026-09-25 | Edge Function `ocr-worker` no staging: FEITO em 2026-09-25 pelo orquestrador: função implantada pelo MCP (v1, ACTIVE, `verify_jwt=false`, 31 arquivos de `supabase/functions`); `pg_cron`, `pg_net`, segredos `ocr_worker_url`/`ocr_worker_secret`/`audit_ip_pepper` no Vault (o humano gravou o `ocr_worker_secret` e a URL). Um tick de teste (`net.http_post` a partir do banco, lendo o Vault) chegou à função e recebeu `401 unauthorized`: o `WORKER_SHARED_SECRET` dos secrets da função não confere com o `ocr_worker_secret` do Vault (ou não está definido na função). Sem ler segredos não dá para saber qual; o job `ocr-worker-tick` continua INATIVO para não gerar 401 por minuto. Falta (humano, painel): reconferir `WORKER_SHARED_SECRET` da função e o `ocr_worker_secret` do Vault (mesmo valor), depois o orquestrador repete o tick (esperado 200) e ativa o job com `cron.alter_job(1, active := true)` | alta | Humano (conferir o valor do secret) → orquestrador | em andamento (função no ar; falta o segredo bater) |
