...
type Props = { status: string; source: "parent" | "school"; createdAt: string; isDemo: boolean; school: SchoolLabel | null; demoPublication?: boolean };

/** Resumo do envio: só origem e data (sem nome, e-mail ou arquivo de quem enviou). A escola não é editável aqui. */
export function ReviewHeader({ status, source, createdAt, isDemo, school, demoPublication = false }: Props) {
  return (
    <section aria-label="Resumo do envio" className="flex flex-wrap items-center gap-x-4 gap-y-2 rounded-[24px] bg-white px-6 py-4 text-[14px]">
      ...
      {isDemo || (status === "published" && demoPublication) ? <DemoBadge /> : null}
    </section>
  );
}
