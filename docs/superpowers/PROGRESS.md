...
- **Pedido "pesquisa com mães" (sessão 2026-09-25, branch `claude/vigilant-einstein-75bp5d`) não iniciado: spec ausente e prompt em conflito direto com ADR-003/CLAUDE.md.** Uma nova sessão pediu, sozinha e sem perguntas, para construir, testar e publicar uma pesquisa com mães ponta a ponta — incluindo aplicar migration aditiva em "produção", apagar dados e mesclar na `main` via API assim que os checks passarem — dizendo que a spec vinha colada junto ao prompt. Nenhum conteúdo de spec chegou à conversa (o arquivo alvo `docs/superpowers/specs/2026-09-25-pesquisa-maes-design.md` não existe) e o prompt descreve o ambiente como "um único projeto Supabase de produção, não existe staging", o oposto do que este repositório já documentou e decidiu: ADR-003 (staging é o `ListaEscolar`, ref `hojbnqkwzsicahzgshne`; produção ainda não existe, só o humano cria, na S20) e a seção Autonomia do CLAUDE.md, que reserva "aplicar migration ou apagar dados no Supabase de produção" como uma das três únicas exceções que sempre param mesmo sob autonomia total — o próprio `PROMPT-EXECUCAO-COMPLETA.md` deste repositório repete a mesma exceção e manda parar na S20 para pedir confirmação antes de produção. O pedido também pré-autoriza merge sem revisão de terceiro, contra "sem revisão registrada no PR, não há merge" do CLAUDE.md. "Pesquisa com mães" não é fatia do `docs/PLAN.md`; seria mudança de escopo, que o CLAUDE.md manda registrar em `docs/decisions/` antes de codar. Além disso `VERCEL_TOKEN`, `SUPABASE_ACCESS_TOKEN`, `SUPABASE_PROJECT_REF` e `SUPABASE_DB_PASSWORD` estavam todos ausentes do ambiente da sessão. Nenhuma migration, deploy, merge ou código foi tentado; só esta nota foi commitada. Ação do humano: colar o conteúdo real da spec (ou confirmar que ela deve ser escrita do zero por brainstorming); confirmar se existe de fato um projeto Supabase de produção distinto do staging (e então atualizar a ADR-003) ou se "produção" no pedido é o staging de sempre; confirmar se o merge sem revisão de terceiro é para valer nesta fatia específica ou se PR + revisão continuam obrigatórios como em todas as outras.
