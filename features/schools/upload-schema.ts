...
/** 4 MB decimais (< 4 MiB): cabe no `serverActions.bodySizeLimit` de 4mb com a sobra do multipart. Maiores: `pnpm import:inep`. */
...
export const uploadFileSchema = z
  ...
  .superRefine((f, ctx) => {
    ...
    else if (f.size > MAX_UPLOAD_BYTES) ctx.addIssue({ code: "custom", message: "O arquivo excede 4 MB. Arquivos maiores entram pelo script de importação." });
