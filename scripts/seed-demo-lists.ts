 ...
 * Uso: pnpm seed:demo-lists (sem opções; não há flag para produção)
...
async function main(): Promise<void> {
  ...
  for (const plan of DEMO_LIST_PLANS) {
    const { data: school, error: se } = await client
      .from("schools")
      .select("id, is_demo")
      .eq("inep", plan.inep)
      .maybeSingle();
    ...
    if (!school?.is_demo)
      throw new Error(`Escola demo ${plan.inep} não encontrada: rode o import:inep --demo antes.`);
    const { data: grade, error: ge } = await client
      .from("grades")
      .select("id")
      .eq("slug", plan.gradeSlug)
      .maybeSingle();
    ...
    if (existing) {
      ...
      if (ce) throw new Error(`Consulta de versões falhou (${ce.code})`);
      assertExistingListMatches(plan, {
    ...
    const listId = await repo.createDraftList({
      schoolId: school.id,
      gradeId: grade.id,
      schoolYear: year,
      isDemo: true,
    ...
    for (const to of ["submitted", "processing", "approved"] as const)
      await repo.transition({ listId, to, actorId });
    if (plan.publish) {
