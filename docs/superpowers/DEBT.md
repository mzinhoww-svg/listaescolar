...
| D-002 | ledger-pipeline S07 onda final | Envio de lista pela escola usa `profiles.role = school_member` sem conferir o vínculo em `school_members`; uma escola pode enviar em nome de outra (a revisão humana antes de publicar cobre o intervalo) | alta | S11 | resolvida em 2026-09-25 (S11, PR #29): `submissions_create` recusa envio de escola sem vínculo em `school_members` (42501 `school_not_linked`); tela de escola usa só escolas vinculadas |
...
| D-065 | ledger-pipeline S09 (M-2) | `ocr-worker`: o ramo sem pipeline responde 500 `misconfigured`/`pipeline_unavailable` mesmo varrendo publicação; sem regressão em deploys hospedados | baixa | S11 | resolvida em 2026-09-25 (S11, PR #29): worker com pipeline nulo continua varrendo a publicação (teste com alias do supabase-js) |
| D-066 | ledger-pipeline S09 (M-7) | Quando a porta publica e o envio já não está `approved` (`not_approved`), a versão fica órfã sem linha persistente própria; só alerta `published_not_recorded` | média | S11 | resolvida em 2026-09-25 (S11, PR #29): `publication_reconcile_orphan` + botão "Conciliar publicação"; `review_complete_publish` grava `publish_orphaned` em vez de silêncio |
...
| D-068 | ledger-pipeline S09 (pendência) | Regra 5 só bloqueia lista-alvo `archived`; lista em `human_review`/`review_needed` passa pelo motor | média | S11 | resolvida em 2026-09-25 (S11, PR #29): `list_publish_from_pipeline` recusa lista fora de draft/approved/published com `list_state_conflict` (vai à revisão humana) |
...
| D-087 | ledger-comercio S11 T5 | E2E da S11 não exercita pela UI: push com coletor local, "Aprovar e publicar" do admin com `review_assign_school`, cópia do pai em `/carrinho/novo?lista=<copyId>` (dono vs. outro pai) e "Conciliar publicação"; cobertos por testes de banco/unidade; repetir no preview com VAPID de staging | média | S20 | aberta |
| D-088 | ledger-comercio S11 T5 | Sem VAPID e sem e-mail no ambiente do E2E, os canais externos ficam "indisponível"; a entrega real ao FCM/Mozilla/Apple nunca foi exercitada (só o `WebPushNotifier` com chaves em memória) | média | Humano (VAPID de staging) / S20 | aberta |
| D-089 | ledger-comercio S11 T5 (E2E) | `list_publish_from_pipeline` cria `school_lists.is_demo` a partir do ENVIO, não da escola: envio real de escola `is_demo` gera lista `is_demo=false` (a página pública mostra o selo pela escola, mas carrinho/lead derivam `is_demo` da lista); a lista de escola demo deveria nascer demo ou `demo_mismatch` comparar com a escola | média | S19 | aberta |
| D-090 | ledger-comercio S11 T5 | `pnpm test:db` supõe banco recém-`db:reset`; rodar depois de um E2E que semeia dados dá 23 falhas ambientais. Documentar ou fazer a suíte limpar o que usa | baixa | S18 | aberta |
| D-091 | ledger-comercio S11 T5 | Roteiros E2E (S09, S10, S11) repetem ~40 linhas de helpers (`ab`, `sql`, `login`, `wait_text`, `clicktext`); extrair `scripts/e2e-lib.sh` | baixa | S18 | aberta |
| D-092 | revisão final S11 (PR #29) | `NotificationBell` é estático: o plano da Task 4 previa atualização leve da contagem a cada 60 s; hoje só atualiza ao navegar | baixa | S18 | aberta |
| D-093 | advisors do staging (2026-09-25, após 0602) | View `public.stationery_public` definida com SECURITY DEFINER (advisor ERROR): a view roda com as permissões do criador, não do consultante; trocar por `security_invoker = true` (PG15+) ou garantir que só expõe colunas públicas da papelaria ativa | média | S19 (revisão de segurança) | aberta |
| D-094 | advisors do staging (2026-09-25) | Extensão `pg_net` instalada no schema `public` (advisor WARN); mover para `extensions` e ajustar o job `ocr-worker-tick` (`net.http_post`) | baixa | S19 | aberta |
| D-095 | advisors do staging (2026-09-25) | Proteção contra senha vazada (HaveIBeenPwned) desligada no Auth do staging (advisor WARN); ligar no painel (humano) no staging e na produção; o produto usa link mágico, impacto baixo | baixa | Humano (painel) / S20 | aberta |
