...
declare
  ...
  v_nb text := nullif(lower(btrim(coalesce(p_neighborhood, ''))), '');
...
begin
  ...
  if p_cart_id is null or not exists (select 1 from public.carts k where k.id = p_cart_id and k.owner_id = p_requester_id) then
  ...
  v_served := s.municipality_id = p_municipality_id
    or exists (select 1 from public.stationery_areas a where a.stationery_id = s.id and a.municipality_id = p_municipality_id);
  if v_served and v_nb is not null then
    v_served := exists (select 1 from public.stationery_areas a
                         where a.stationery_id = s.id and a.municipality_id = p_municipality_id and a.neighborhood = v_nb)
      or (s.municipality_id = p_municipality_id and lower(btrim(coalesce(s.neighborhood, ''))) = v_nb)
      -- papelaria sem bairro nem área no município atende o município inteiro
      or (s.municipality_id = p_municipality_id and btrim(coalesce(s.neighborhood, '')) = ''
          and not exists (select 1 from public.stationery_areas a where a.stationery_id = s.id and a.municipality_id = p_municipality_id));
  end if;
  ...
  loop
    ...
    begin
      insert into public.leads (
        ...
        p_idempotency_key, coalesce(p_is_demo, false)
...
-- authenticated: sem requester_id, consent_* e idempotency_key (a papelaria nunca identifica o responsável);
-- a RLS ainda restringe as linhas. Sem INSERT/UPDATE/DELETE/TRUNCATE para ninguém.
grant select (id, code, cart_id, list_id, stationery_id, status, school_name, grade_label, school_year, municipality_id,
...
-- lead_events: sem actor_id.
grant select (id, lead_id, event_type, from_status, to_status, actor_role, amount_cents, reason, item_count, created_at, updated_at)
