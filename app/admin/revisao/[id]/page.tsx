...
import { buildReviewService, loadSchoolLabels, loadThresholds } from "../loaders";
...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [thresholds, labels, blockers] = await Promise.all([
    ...
    reviewing ? buildReviewService().blockers(actor, s.id, false).catch(() => []) : Promise.resolve([]),
  ]);
  ...
  return (
    <AdminShell {...shell} actions={<Link href="/admin/revisao" className="inline-flex min-h-11 items-center text-[14px] font-extrabold underline">Voltar à fila</Link>}>
      <ReviewHeader status={s.status} source={s.source} createdAt={s.createdAt} isDemo={s.isDemo} school={s.schoolId ? (labels[s.schoolId] ?? null) : null} />
      <DraftProvider>
