...
- Ruling: `/carrinho/**` e `/ir-para/**` não são prefixos protegidos do proxy (S02); as páginas exigem login por `getCurrentUser` + redirect para `/entrar?next=`, e o handler `go` responde 307 ao login. Dono verificado explicitamente (`ownerId === user.id`), além da RLS (admin lê tudo pela RLS, mas não abre carrinho alheio) — custo se errada: baixo.
- Ruling: falha ao gravar `affiliate_clicks` no `go` responde 500 sem redirecionar (o registro do clique é requisito; sem ele não há atribuição) — custo se errada: baixo (trocar por redirecionar mesmo assim).
